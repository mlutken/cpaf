...
class path(_base):
    ...
    def makedirs(self, bIgnoreIfExist=0, mode=0o777):
        if os.path.exists( self ) and bIgnoreIfExist: 
            return
        else:
    ...
    def removedirs(self, bIgnoreIfNotExist=0 ):
        if not os.path.exists( self ) and bIgnoreIfNotExist: 
            return
        else:
            os.removedirs(self)

    def removetree(self, bIgnoreIfNotExist=0 ):
        if not os.path.exists( self ) and bIgnoreIfNotExist: 
            return
        else:
            shutil.rmtree(self)
