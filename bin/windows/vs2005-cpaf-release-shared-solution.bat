...
for /f "delims=" %%i in ('sed "s/vs2005-cpaf-release-shared-solution.bat//g" sourcePath.txt') do set ScriptSourcePath=%%i
del sourcePath.txt
...
rem --- Release Shared ---
rem --------------------------
echo Creating Release Shared VS2005 solution ...
SET BUILD_DIR=build_cbs\vc8_release_shared
chdir /d %ScriptSourcePath%
cd ../..
mkdir  %BUILD_DIR%
chdir %BUILD_DIR%
cmake -D CMAKE_BUILD_TYPE:STRING=Release -D CBS_BUILD=ON -D CBS_LIB_TYPE_TO_BUILD:STRING=SHARED -G "Visual Studio 8 2005" ../..
chdir /d %SaveDir%
