...
for /f "delims=" %%i in ('sed "s/vs2005-cpaf-debug-static-solution.bat//g" sourcePath.txt') do set ScriptSourcePath=%%i
del sourcePath.txt
...
rem --- Debug Static ---
rem ------------------------
echo Creating Debug Static VS2005 solution ...
SET BUILD_DIR=build_cbs\vc8_debug_static
chdir /d %ScriptSourcePath%
cd ../..
mkdir  %BUILD_DIR%
chdir %BUILD_DIR%
cmake -D CMAKE_BUILD_TYPE:STRING=Debug -D CBS_BUILD=ON -D CBS_LIB_TYPE_TO_BUILD:STRING=STATIC -G "Visual Studio 8 2005" ../..
chdir /d %SaveDir%
