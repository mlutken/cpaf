...
for /f "delims=" %%i in ('sed "s/vs2005-cpaf-release-static-solution.bat//g" sourcePath.txt') do set ScriptSourcePath=%%i
del sourcePath.txt
...
rem --- Release Static ---
rem -------------------------
echo Creating Release Static VS2005 solution ...
SET BUILD_DIR=build_cbs\vc8_release_static
chdir /d %ScriptSourcePath%
cd ../..
mkdir  %BUILD_DIR%
chdir %BUILD_DIR%
cmake -D CMAKE_BUILD_TYPE:STRING=Release -D CBS_BUILD=ON -D CBS_LIB_TYPE_TO_BUILD:STRING=STATIC -G "Visual Studio 8 2005" ../..
chdir /d %SaveDir%
