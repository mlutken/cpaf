...
bool video_render_thread::state__normal_flow(av_frame& current_frame, render& video_render)
{
    if (!current_media_time().time_is_paused()) {
        const auto media_time_float = cpaf::time::to_float_seconds(current_media_time().video_time_pos());
        const auto frame_time_float = cpaf::time::to_float_seconds(current_frame.presentation_time());
        if (media_time_float >= frame_time_float) {
            current_pipeline_index_ = current_frame.pipeline_index();
//          if (current_media_time().video_time_pos() >= current_frame.presentation_time()) {
...
{
    if (
       (false && video_frame_update_dbg_counter_ % 200000 == 0)
    )
    {
        ...
        std::cerr
                ...
                << ", video time: " << current_frame.presentation_time_ms().count() << " ms"
                << "\n";
    }
