...
public:
    ...
    uint64_t			ff_audio_channel_layout		() const { return ff_codec_context_->channel_layout; }
    ...
    uint64_t			channel_layout				() const { return ff_codec_context_->channel_layout; }
...
//	const std::string& resource_path	() const { return resource_path_; }
    ...
    ///av_frame            read_frame_for_time_pos (const std::chrono::microseconds& next_) const;
    int					send_packet				(const av_packet& packet) const;
    int					receive_frame			(av_frame& frame) const;
//    int					receive_frame			() const;
