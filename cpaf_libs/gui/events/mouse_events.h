...
struct mouse : public base
{
    ...
    * @brief Mouse wheels
    */
    enum class wheel_direction : std::int8_t
    {
    ...
    static std::string  to_name     (mouse::wheel_direction w);
    static std::string  to_name     (mouse::scroll_direction dir);
    ...
    std::int32_t            xrel            = 0;                        //!< Relative motion in the X direction. For wheel event: The amount scrolled horizontally, positive to the right and negative to the left
    std::int32_t            yrel            = 0;                        //!< Relative motion in the Y direction. For wheel event: The amount scrolled vertically, positive away from the user and negative toward the user
    mouse::button           btn             = mouse::button::left;      //!< Which button
    mouse::wheel_direction  wheel_dir       = mouse::wheel_direction::vertical;   //!< Which wheel (for mice with multiple ones)
    //    float                   wheel_x     = 0;                        //!< The amount scrolled horizontally, positive to the right and negative to the left, with float precision
    //    float                   wheel_y     = 0;                        //!< The amount scrolled vertically, positive away from the user and negative toward the user, with float precision
    scroll_direction        scroll_dir      = scroll_direction::normal;  //!< Wheel offset (positive is up/left, negative is down/right). High-precision mice may use non-integral offsets.
